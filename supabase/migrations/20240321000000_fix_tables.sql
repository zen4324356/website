...
DROP TABLE IF EXISTS server_storage_stats CASCADE;
DROP TABLE IF EXISTS server_emails CASCADE;
...
ALTER TABLE server_storage_stats ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Enable read access for all users" ON server_storage_stats
    FOR SELECT USING (true);

CREATE POLICY "Enable read access for all users" ON server_emails
    FOR SELECT USING (true);

CREATE POLICY "Enable read access for all users" ON access_tokens
    FOR SELECT USING (true);

CREATE POLICY "Enable read access for all users" ON google_configs
    FOR SELECT USING (true);

-- Create indexes for better performance
CREATE INDEX idx_server_emails_domain ON server_emails(domain);
CREATE INDEX idx_server_emails_created_at ON server_emails(created_at);
CREATE INDEX idx_access_tokens_token ON access_tokens(access_token);
CREATE INDEX idx_google_configs_client_id ON google_configs(client_id); 